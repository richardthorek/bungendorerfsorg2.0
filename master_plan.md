...
| PR 7 config — set `AZURE_OPENAI_ENDPOINT` / `AZURE_OPENAI_API_KEY` / `AZURE_OPENAI_DEPLOYMENT` (a vision-capable chat deployment) / `AZURE_OPENAI_API_VERSION` on the SWA                                                  | Pending — owner                       |
