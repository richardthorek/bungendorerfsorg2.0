...
| Members' area    | `public/admin.html` + `public/js/admin.js` + `api/auth-*` + `api/members` | Passwordless sign-in (ACS email code, 60-min session), `@rfs.nsw.gov.au` allow-list in `brfsstorage`. Shared logic in `api/shared/` (`handlers.js`, `auth.js`, `store.js`, `identity.js`, `otpEmail.js`), mirrored in `server.js`. Seed first admin with `scripts/seed-member.js` |
