...
| Members' area    | `public/admin.html` + `public/js/admin.js` + `api/{auth-*,members,duty,content,enquiries}` | Passwordless sign-in (ACS email code, 60-min session), `@rfs.nsw.gov.au` allow-list in `brfsstorage`. Runs the **Brigade phone** (call/SMS forwarding number, replaces the SharePoint lookup) and **Events & training** editors (home-page widgets, replaces the M365 calendar feed). Also an **Enquiries** list (contact-form submissions land here + are emailed). Shared logic in `api/shared/`, mirrored in `server.js`. Seed: `scripts/seed-{member,duty,content,enquiries}.js` |
